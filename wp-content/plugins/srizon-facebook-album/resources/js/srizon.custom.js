...
function srizon_trigger_js() {
    if (jQuery().magnificPopup) {
        ...
        jQuery('.jtubegallery').each(function () {
            ...
            // console.log($this.data('disablelb'));
            jQuery(this).magnificPopup({
                delegate: '.magpopif',
                type: 'iframe',
    ...
    if (jQuery('.collage-layout').length) {
        jQuery('.collage-layout').each(function () {
            ...
            if (jQuery(this).data('hovercaption') !== '') {
                beforeCaption.collageCaption({behaviour_c: jQuery(this).data('hovercaption')})
            }
        });
    }
...
function load_juser_video(scrollerid, videoid, autoplay) {
    ...
    embedcode = '<div>' +
        ...
        '<tr>' +
        '<td>' +
        '<div class="juser-vid-container"><iframe class="youtube-player" type="text/html" width="1920" height="1200" src="//www.youtube.com/embed/' + videoid + '?fs=1&rel=0&wmode=transparent' + autotext + '" frameborder="0" allowfullscreen></iframe></div>' +
        '</td>' +
        '</tr>' +
        '</table>' +
        '</div>';
